/* =================================================================
NFL SCRIPT 2 - SEASON TRENDS TAB (AI-ENHANCED) ANF LEAGUE AVerages
==================================================================*/
...
(() => {
  ...
  function projectRange(values) {
    ...
    const expected = clamp(avg * (1 + yoy));
    const floor = clamp(expected * 0.85);
    const ceiling = clamp(expected * 1.15);

    return { floor, expected, ceiling, yoy };
  }

  function confidenceFromYoY(yoy) {
  ...
  function positionStats(pos, seasons) {
    ...
    const map = {
        ...
        "Yards / Attempt": seasons.map(s => clamp(s.passing_yards / s.attempts)),
        ...
        "Yards / Carry": seasons.map(s => clamp(s.rushing_yards / s.carries)),
...
function renderSeasonPredictions(seasons) {
  ...
  const stats = positionStats(pos, seasons);
  ...
  const rows = Object.entries(stats).map(([stat, values]) => {
    ...
    const confidence = confidenceFromYoY(yoy);
