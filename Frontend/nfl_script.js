...
function renderPlayerCharts() {
  ...
  const statMap = {
    QB: [
      ...
      { canvas: "chart7", title: "Fantasy Football Points", key: "fantasy_points" }
    ],
    RB: [
  ...
  statMap[position].forEach(stat => {
    ...
    charts[stat.canvas] = new Chart(ctx, {
      ...
      data: {
        ...
        datasets: [{ label: playerName, data: [Number(player[stat.key] || 0)], backgroundColor: "#4C72B0", maxBarThickness: 80, categoryPercentage: 0.7, barPercentage: 0.8 }]
      },
      options: {
        responsive: true,
        ...
        plugins: { 
          title: { display: true, text: stat.title, padding: { top: 10, bottom: 35 }, color: "#fff" },
          ...
          datalabels: {
            ...
            formatter: value => {
              if (value === 0) return ""; //hides the zero limits
              return value;
            }
          }
        },
...
function safeRate(numerator, denominator, decimals = 2, suffix = "") {
  ...
  {
    label: "Avg Fantasy Points per Game",
    calc: p => safeRate(p.fantasy_points, p.games)
  }
],
...
function renderCompareCharts() {
  ...
  const statMap = {
    QB: [
      ...
      { canvas: "compareChart7", title: "Fantasy Football Points", key: "fantasy_points" }
    ],
    RB: [
  ...
  statMap[position].forEach(stat => {
    ...
    charts[stat.canvas] = new Chart(ctx, {
      ...
      data: {
        labels: [stat.key.replace(/_/g, " ").toUpperCase()],
        datasets: [
          { label: p1Name, data: [Number(p1[stat.key] || 0)], backgroundColor: "#4C72B0", maxBarThickness: 80 },
          { label: p2Name, data: [Number(p2[stat.key] || 0)], backgroundColor: "#22d3ee", maxBarThickness: 80 }
        ]
      },
      options: {
        responsive: true,
